...
def create_authentication_blueprint(account_repository, authenticator,
      account_registration_controller, password_reset_controller):
  ...
  def post_register_request():
    ...
    result = account_registration_controller.initiate_registration(email,
                                                                   g.locale)
    flash(g.locale.text(
      u'An email containing your registration form link has been sent to your email address.'),
          'success')
  ...
  @require_logout
  def post_password_reset_link_request():
    email = request.form['email']
    password_reset_controller.initiate_password_reset(email, g.locale)
    flash(g.locale.text(
      u"An email has been sent. If you have't received it in a few minutes, please check your spam folder."),
          'success')
  ...
  def post_password_reset_form(token):
    ...
    if result == 'success':
      ...
      return redirect(
        url_for('authentication.get_password_reset_form', token=token))
  ...
  def post_login():
    ...
    if Username.is_valid(posted_username) and Password.is_valid(
          posted_password):
