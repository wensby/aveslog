...
from birding.bird_resource import BirdResourceAccessor, BirdResource
...
class TestBirdViewFactory(TestCase):
  ...
  def setUp(self):
    ...
    self.factory = BirdResourceAccessor(
  ...
  def test_create_bird_page_view_returns_correct_page_view(self):
    ...
    page_view = self.factory.access_bird(bird_id=4)

    self.assertEqual(page_view, BirdResource(bird, picture, picture))
  ...
  def test_eq_false_when_other_type(self):
    ...
    page_view = BirdResource(bird, cover_picture, thumbnail_picture)
    self.assertNotEqual(
      page_view,
      'BirdPageView('
      'Bird(4, Pica pica,) '
